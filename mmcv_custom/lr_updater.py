...
class StepstageLrUpdaterHook(StepLrUpdaterHook):
    """ use inner epochs of one stage to get lr"""
    def __init__(self, step, gamma=0.1, **kwargs):
        assert isinstance(step, list), '"step" must be a list of list'
        for stage in step:
            assert isinstance(stage, list), '"step" must be a list of list'
            for s in stage:
                assert isinstance(s, int) and s > 0
        self.step = step
        self.gamma = gamma
    ...
    def get_lr(self, runner, base_lr):
        progress = runner.stage_epoch if self.by_epoch else runner.stage_iter
        step = self.step[runner.current_stage]

        if isinstance(step, int):
            return base_lr * (self.gamma**(progress // step))
