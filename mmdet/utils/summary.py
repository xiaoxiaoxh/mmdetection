...
def is_custom_operator(module):
    return any(
        [isinstance(module, custom_op) for custom_op in custom_operator_list])
...
def summary(model, cfg):
        ...
        def hook(module, input, output):
            ...
            if class_name == "ReLU":
                ...
                if getattr(module, "affine"):
                    flops *= 2
            elif class_name == "Conv2d" or class_name == "ConvTranspose2d" \
                    or class_name == "DeformConv":
                flops = params * torch.prod(torch.LongTensor(output_shape[1:]))
            elif class_name == "MaxPool2d":
                if isinstance(module.kernel_size, tuple):
                    kernel_ops = torch.prod(
                        torch.LongTensor([*module.kernel_size]))
                else:
                    kernel_ops = torch.prod(
                        torch.LongTensor([module.kernel_size**2]))
                flops = kernel_ops * torch.prod(torch.LongTensor(output_shape))
            elif class_name == "AdaptiveMaxPool2d":
                kernel_ops = torch.prod(
                    torch.LongTensor(input_shape[1:]) //
                    torch.LongTensor(output_shape[1:]))
                flops = kernel_ops * torch.prod(torch.LongTensor(output_shape))
            elif class_name == "AvgPool2d":
                if isinstance(module.kernel_size, tuple):
                    kernel_ops = torch.prod(
                        torch.LongTensor([*module.kernel_size])) + 1
                else:
                    kernel_ops = torch.prod(
                        torch.LongTensor([module.kernel_size**2])) + 1
                flops = kernel_ops * torch.prod(torch.LongTensor(output_shape))
            elif class_name == "AdaptiveAvgPool2d":
                kernel_ops = torch.prod(
                    torch.LongTensor(input_shape[1:]) //
                    torch.LongTensor(output_shape[1:])) + 1
                flops = kernel_ops * torch.prod(torch.LongTensor(output_shape))
            elif class_name == "Softmax":
                flops = torch.prod(torch.LongTensor(output_shape)) * 3
            elif class_name == "NonLocal2d":
                flops = 2 * (torch.prod(torch.LongTensor(output_shape[1:])) **
                             2) * module.planes
    ...
    for m_name, m in model.named_modules():
        ...
        if (not isinstance(m, nn.Sequential)
                and not isinstance(m, nn.ModuleList) and not (m == model) and
            (hasattr(nn, m.__class__.__name__) or is_custom_operator(m))):
    ...
    line_new = line_new_format.format("Layer (type)", "Output Shape",
                                      "Param #", "FLOPS")
    ...
    s += "Non-trainable params: {0:,}".format(total_params -
                                              trainable_params) + "\n"
    s += "-" * line_length + "\n"
